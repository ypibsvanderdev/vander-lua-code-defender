                            ...
                            <div>
                                ...
                                <div style={{ border: '1px solid var(--border-color)', borderRadius: '6px', overflow: 'hidden' }}>
                                    <div style={{ background: 'var(--bg-secondary)', padding: '14px 20px', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: '10px', fontSize: '14px' }}>
                                        ...
                                        <span style={{ color: 'var(--text-secondary)' }}>{selectedRepo.commits?.[0]?.msg || 'Initial commit'}</span>
                                        <span style={{ marginLeft: 'auto', color: 'var(--text-secondary)', fontSize: '12px' }}>{selectedRepo.commits?.[0]?.hash || '---'} · {selectedRepo.commits?.[0]?.time || 'Just now'}</span>
                                    </div>
                                    {selectedRepo.files.map(f => (
                                        <div key={f.name} className="nav-item" style={{ padding: '12px 20px', borderBottom: '1px solid var(--border-color)', borderRadius: 0, gap: '14px' }}
                                            ...
                                            <span style={{ color: 'var(--text-secondary)', fontSize: '12px' }}>{selectedRepo.commits?.[0]?.msg || 'Initial commit'}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>

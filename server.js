...
app.post('/api/deobfuscate', (req, res) => {
    ...
    try {
        ...
        // --- UNIVERSAL DEOBFUSCATION PATTERNS ---

        // 1. Decode Escaped Characters (\123 or \xAB)
        currentCode = currentCode.replace(/\\(\d{1,3})/g, (match, dec) => String.fromCharCode(parseInt(dec)));
        currentCode = currentCode.replace(/\\x([0-9A-Fa-f]{2})/g, (match, hex) => String.fromCharCode(parseInt(hex, 16)));

        // 2. Clear Junk Comments & Formatting
        currentCode = currentCode.replace(/--\[\[[\s\S]*?\]\]/g, ''); // Block comments
        currentCode = currentCode.replace(/--.*$/gm, ''); // Line comments

        // 3. Reconstruct string.char sequences
        // Pattern: string.char(104, 101, 108, 108, 111)
        const charPattern = /string\.char\s*\(([\d,\s]+)\)/g;
        currentCode = currentCode.replace(charPattern, (match, chars) => {
            const decs = chars.split(',').map(c => String.fromCharCode(parseInt(c.trim())));
            return `"${decs.join('')}"`;
        });

        // 4. Constant Table Reconstruction (Common in Luraph/IronBrew)
        const tablePattern = /local\s+(\w+)\s*=\s*\{([\d,\s]+)\}/g;
        let match;
        while ((match = tablePattern.exec(currentCode)) !== null) {
            const varName = match[1];
            const vals = match[2].split(',').map(v => parseInt(v.trim()));
            if (vals.every(v => v >= 0 && v <= 255)) {
                const reconstructed = vals.map(v => String.fromCharCode(v)).join('');
                // If it looks like code, we don't wrap in quotes, otherwise we do
                const replacement = reconstructed.includes('local') || reconstructed.includes('function') ? reconstructed : `"${reconstructed}"`;
                const usagePattern = new RegExp(`table\\.concat\\(${varName}\\)`, 'g');
                currentCode = currentCode.replace(usagePattern, replacement);
            }
        }

        // 5. Handle VanderHub Specific Layers (Recursive Check)
        const extractLayerData = (src) => {
            ...
            let m;
            while ((m = chunkRegex.exec(src)) !== null) {
                const vals = m[1].split(',').map(v => parseInt(v.trim()));
                bytes.push(...vals);
            }
            return bytes.length > 0 ? { key, bytes } : null;
        };

        const l2 = extractLayerData(currentCode);
        if (l2 && currentCode.includes('Shield')) {
            let l1 = "";
            for (let b of l2.bytes) l1 += String.fromCharCode((b - l2.key + 256) % 256);
            currentCode = l1;
            // Recurse once for XOR layer
            const l1data = extractLayerData(currentCode);
            if (l1data) {
                let orig = "";
                for (let b of l1data.bytes) orig += String.fromCharCode(b ^ l1data.key);
                currentCode = orig;
            }
        }

        // 6. Final Aesthetic Clean (Removing excessive newlines)
